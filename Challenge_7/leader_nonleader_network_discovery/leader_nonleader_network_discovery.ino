...
const int UID = 1;               // UID of the current node - do not use UID = 2 (to distinguish network discovery blasts)
...
const int blink_LED = 1000;        // If LED blinked, lenght of on or off cycle
...
int leader_threshold = 5;        // max number of iterations without hearing from leader before declaring self leader
...
//int leader_current_node = 0;      // if the current node is the leader, this value is 1, starts at 0 unless debugging
//int leader_time = 0;              // number of iterations since last heard from leader
//int leader_threshold = 30 + UID;        // max number of iterations without hearing from leader before declaring self leader
//int leaderPingCNT = 0;            // Leader Pings Network every leaderPingThreshold loops
//int leaderPingThreashold = 10;    // Number of loops through leader code before pinging network
...
void loop() 
{
  // Determine whether to run leader or non-leader code
    ...
    if (leader_time > leader_threshold){
      ...
      // If the leader has gone silent, I elect myself leader
      leader_current_node = 1;
    }
    ...
    if (int_rx == LeaderBroadcastInt){
      Serial.println("Resetting Leader Time");
      leader_time = 0;
    }
...
//    }
      ...
      Serial.println("Infected by message");
      infected = 1;
    }
    else if (int_rx == InfectionClearInt){
      // Clear Infection
      infected = 0;
      Serial.println("Resetting Leader Time");
      leader_time = 0;
      ...
      if(buttonPress == 0){
        Serial.println("Infected by button press");
        // Get Infected
        infected = 1;
        // Reset LEDs
        digitalWrite(RED_LED, HIGH);
...
//    }
    ...
    else if (int_rx == LeaderBroadcastInt){
      // Leader heard from another leader, unelects self
      leader_current_node = 0;
      leader_time = 0;
    }
