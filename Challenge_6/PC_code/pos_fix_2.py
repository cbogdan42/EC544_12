...
position=[30,30]
# Beacon Locations
Beacons = {0:[100,501],1:[501,501],2:[300,100]}
# Colors
white=[255,255,255]
black = [0,0,0]
...
def GetRSSI():
    RSSI_file = open('RSSI_data.txt','r')
...
def GetBin():
    ...
    for x in range(len(rssi)): 
        ...
        for y in range(3):
            ...
            temp.append(math.sqrt((pow((rssi[x][y]-test[0][y+1]),2))))
...
#    sleep(100)
    ...
    for x in range(k):
        position.append(rssi[sortedind[x]][3])
    #print (position)
...
def GetDistance():
    ...
    print('incoming vals')
    print(current_RSSI_hold)
    current_RSSI.append(int(current_RSSI_hold[2:4]))
    current_RSSI.append(int(current_RSSI_hold[5:7]))
    current_RSSI.append(int(current_RSSI_hold[8:10]))
...
#        print(list_beacons)
    ...
    for n in beacon_vals:
        distance_vals.append(50.0/(1-(n/70.0))-15.0)
        
    print('distance_vals')
    print(distance_vals)
#    print('Beacons')
#    print(Beacons)
    # Beacons[list_beacons[index][x-1,y-2]]

    #print(matrix_1.dtype)
    Mat_1 = np.zeros([2,2])
    Mat_1[0][0] = 501.0-100.0
    Mat_1[0][1] = 501.0-501.0
    Mat_1[1][0] = 300.0-501.0
    Mat_1[1][1] = 100.0-501.0
    #print(Mat_1)

    Mat_2 = np.zeros([2,1])
    Mat_2[0][0] = distance_vals[0]**2 - distance_vals[1]**2 - 100.0**2 + 501.0**2 - 501.0**2 + 501.0**2
    Mat_2[1][0] = distance_vals[1]**2 - distance_vals[2]**2 - 501.0**2 + 300.0**2 - 501.0**2 + 100.0**2
    #print(Mat_2)

    #Mat_1 = np.array([matrix_1],[matrix_2])
    
    #Mat_1 = np.array([Beacons[list_beacons[1]][0]-Beacons[list_beacons[0]][0], \
                       #Beacons[list_beacons[1]][1]-Beacons[list_beacons[0]][1]],\
                       #[Beacons[list_beacons[2]][0]-Beacons[list_beacons[1]][0], \
                       #Beacons[list_beacons[2]][1]-Beacons[list_beacons[1]][1]])
    
    #Mat_2 = np.matrix([distance_vals[0]**2 - distance_vals[1]**2 \
    #                    - Beacons[list_beacons[0]][0]**2 + Beacons[list_beacons[1]][0]**2 \
    #                    - Beacons[list_beacons[0]][1]**2 + Beacons[list_beacons[1]][1]**2], \
    #                    [distance_vals[1]**2 - distance_vals[2]**2 \
    #                    - Beacons[list_beacons[1]][0]**2 + Beacons[list_beacons[2]][0]**2 \
    #                    - Beacons[list_beacons[1]][1]**2 + Beacons[list_beacons[2]][1]**2])
    
    Current_Position = 0.5*np.dot(np.linalg.inv(Mat_1),Mat_2)
