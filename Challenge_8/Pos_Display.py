...
def GetRSSI():
    ...
    current_RSSI_file = csv.reader(RSSI_file)
    RSSI_file.close()
    print(current_RSSI_file)
    return current_RSSI_file
    
# This function determines the current bin
def GetBin(current_RSSI):
...
def GetDistance():
    ...
    for n in range(len(beacon_vals)):
        print(beacon_vals[n])
        temp = int(beacon_vals[n])
        distance_vals.append(.99/(1.0-temp/70))
        
    # Beacons[list_beacons[index][x-1,y-2]]
    Mat_1 = np.matrix([Beacons[list_beacons[1]][0]-Beacons[list_beacons[0]][0], \
    ...
    Current_Position = 0.5*np.linalg.inv(Mat_1)*Mat_2
